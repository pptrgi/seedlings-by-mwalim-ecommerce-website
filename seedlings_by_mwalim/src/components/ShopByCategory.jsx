    ...
    <div className='pt-[6rem] pb-[2rem]' id='categories'>
        ...
        <div className='container grid grid-cols-1 gap-3 sm:grid-cols-3 px-[3rem] sm:px-[2rem] lg:px-[3rem]'>
            {categoryKeys.map((categoryKey, index) => {
                return (
                    <div key={index} className='relative group bg-whiteColor border-2 shadow-md h-[300px] overflow-hidden lg:h-[320px]'>
                        <img className='absolute bottom-[0.5rem] left-[3rem]' src={categories[categoryKey].image} />
                        <div className='absolute top-0 left-0 p-6 group-hover:opacity-0'>
                            <h3 className='text-textColor text-h2FontSize font-mediumWeight mb-[0.5rem]'>{categories[categoryKey].name}</h3>
                            <p className='text-textColor text-smallerFontSize'>{categories[categoryKey].categorySeedlings.length} items</p>
                        </div>
                        <div className='absolute h-full w-full bg-ctaColor/80 -bottom-10 opacity-0 group-hover:bottom-0 group-hover:opacity-100 transition-all duration-300'>
                            <div className='absolute top-0 left-0 p-6 '>
                                <h3 className='text-whiteColor text-h2FontSize font-mediumWeight mb-[0.5rem]'>{categories[categoryKey].name}</h3>
