...
function CheckoutDetails() {
    ...
    const [totalFigure, setTotalFigure] = useState(null)
    ...
    const handleMpesaPay = e => {
        e.preventDefault()
        setTotalFigure(totalAmount)
        axios.post('https://seedlingsbymwalim.vercel.app/api/token', {
            phone: phoneNumber,
            amount: totalFigure
        }).then(res => {
            setPhoneNumber("")
        ...
        <div className='flex flex-col gap-2 justify-center items-center border-2 border-gray-200 bg-whiteColor px-2 py-4 rounded-[1rem]'>
            <div className='py-[2rem]'>
                <div className='flex justify-between items-center w-[250px] border-b-[1px] border-b-gray-200'>
                    <span className='text-textColor font-mediumWeight text-smallFontSize'>Subtotal</span>
                    <span className='text-titleColor font-mediumWeight text-smallFontSize'>KES {totalAmount}</span>
                </div>
                <div className='flex justify-between items-center w-[250px] my-[2rem] border-b-[1px] border-b-gray-200'>
                    <span className='text-textColor font-mediumWeight text-smallFontSize'>Shipping</span>
                    <div className='flex flex-col space-y-1 items-end'>
                        <span className='text-smallerFontSize text-textColor'>Free delivery around town</span>
                        <span className='text-smallerFontSize text-textColor'>Copia for shipping</span>
                        <span className='text-smallerFontSize text-textColor'>Pickup your package</span>
                    </div>
                </div>
                    ...
                    <div className='w-4/5 flex flex-col justify-center items-center gap-[1rem] px-4'>
                        ...
                        <form onSubmit={handleMpesaPay}>
                            <div className='flex flex-col space-y-[0.25rem] items-start'>
                                <label htmlFor="phoneNumber" className='text-textColor text-smallFontSize font-mediumWeight'>Phone Number</label>
                                <input type="number" name="phoneNumber" value={phoneNumber} onChange={e => setPhoneNumber(e.target.value)} className='bg-whiteColor text-textColor text-smallFontSize px-[0.5rem] py-[0.5rem] border-[2px] border-gray-200 hover:border-ctaColor focus:border-ctaColor focus:outline-none rounded-lg w-full' placeholder='e.g. 0700119134' required/>
                            </div>
                            <div className='mt-[1.5rem] flex justify-center items-center'>
                                <button type='submit' className='bg-green-500 px-[1rem] py-[0.5rem] text-whiteColor text-smallFontSize rounded-full hover:bg-green-600'>{`Pay ${totalAmount} Now`}</button>
                            </div>
